        ...
        threadHistory(params->numThreads)
{
  ...
  unsigned bitsPerWeight = 1 + floorLog2(theta);
...
PathPerceptron::btbUpdate(ThreadID tid, Addr BrAddr, void * &bp_history)
{
    ...
    static long count = 0;
    count++;
    DPRINTFR(PAthPerceptron, "[btbUpdate No.%d] BTB misss resets prediction: %lx\n", count, BrAddr);
#endif
}
...
{
#ifdef RECORD
    DPRINTFR(PAthPerceptron, "[updateHist] Calling updateHist of branch %lx %s...\n", BrAddr, squashed ? "during squash" : "");
#endif
...
PathPerceptron::lookup(ThreadID tid, Addr BrAddr, void * &bp_history)
{
    ...
    static long count = 0;
    count++;
    DPRINTFR(PAthPerceptron, "[lookup No.%d] Looking up branch 0x%lx on index %d: %s\n", count, BrAddr, index, taken ? "taken" : "not taken");
#endif

    // Save non-speculative info into branch-related history
    BPHistory *history = new BPHistory(BrAddr, tHist.Spath, taken, false, out, hislen, tHist.sgHist);
...
PathPerceptron::uncondBranch(ThreadID tid, Addr BrAddr, void * &bp_history)
{
    ...
    static long count = 0;
    count++;
    DPRINTFR(PAthPerceptron, "[uncond No.%d] Unconditional Branch: %lx\n", count, BrAddr);
#endif

    ThreadHistory &tHist = threadHistory[tid];

    BPHistory *history = new BPHistory(BrAddr, tHist.Spath, true, true, 999999, hislen, tHist.sgHist);
                        ...
                        void *bp_history, bool squashed)
{
    ...
    if (!squashed)
        count++;
    DPRINTFR(PAthPerceptron, "[update No.%d] Calling update of branch %lx%s\n", count, BrAddr, squashed ? " during squash" : "");
#endif
    ...
    if (history->taken != taken || abs(history->out) <= theta) {
        ...
        satInc(W[i][0], taken);
#ifdef DEBUG
        DPRINTFR(PAthPerceptron, "W[%3d][0]: %4d -> %4d\n", i, val_prior, W[i][0]);
#endif
...
#ifdef DEBUG
            DPRINTFR(PAthPerceptron, "W[%3d][%3d]: %4d -> %4d\n", k, j, val_prior,  W[k][j]);
#endif
        }
    }
...
#ifdef RECORD
    DPRINTFR(PAthPerceptron, "[squash] Calling squash of branch 0x%lx\n", h->BrAddr);
#endif
    delete h;
}
...
unsigned PathPerceptron::getGHR(ThreadID tid, void *bp_history) const
{
  ...
  static long count = 0;
  count++;
  DPRINTFR(PAthPerceptron, "[getGHR No.%d] Calling getGHR and got val %x\n", count, val);
#endif
  return val;
}
